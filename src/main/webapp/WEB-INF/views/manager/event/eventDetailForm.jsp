				...
				<form role="form" action="eventUpdate" method="post" enctype="multipart/form-data" id="bannerForm">
					<div class="box-body">
						<div class="form-group">
							...
							<input type="text" name="no" class="form-control" value="${eventVO.no }" readonly="readonly">
						</div>
						<div class="form-group">
							<label>이벤트 타이틀</label>
							<input type="text" name="title" class="form-control" value="${eventVO.title }">
						</div>
						<div class="form-group">
							<label>이벤트 설명(내용)</label>
							<textarea rows="15" cols="30" placeholder="내용을 입력하세요" name="content" class="form-control" required="required">${eventVO.content}</textarea>
						</div>
						<div class="form-group">
							<label>저장된 사진</label>
							<div>
...
<script>
      $("#file").change(function(){
	   ...
	   $("button#btnDel").click(function(){
		   ...
		   if(cc){
		   location.href="${pageContext.request.contextPath }/removeEvent?no="+bannerno+"&page=${cri.page}&searchType=${cri.searchType}&keyword=${cri.keyword}";
		   }
	   })
