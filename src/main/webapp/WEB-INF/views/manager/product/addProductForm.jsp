...
<script>
	...
	var getAirData = function(div,res) {
		...
		if(div=='Depature') {
			...
			$(".result").eq(0).append("도착 : "+ res.vo.no+" "+res.vo.ano+" "+res.vo.dlocation+" "+res.vo.rlocation+" "+ddateStr+" "+rdateStr+" "+res.vo.ldiv+" "+res.vo.capacity+" "+res.vo.seat+" "+res.vo.price);
			$("form").append(no);
		}
	}
		...
		$.ajax({
			...
			success : function(res) { // 결과 성공 콜백함수
				...
				$(res.list).each(function(i,obj){
					...
					div=="Depature"?$("#flightDepature #table").append(tr):$("#flightRending #table").append(tr);
				...
				if(res.pageMaker.prev) div=="Depature"?$("#flightDepature .pagination").append(prevli):$("#flightRending .pagination").append(prevli);
				for(var i=res.pageMaker.startPage;i<=res.pageMaker.endPage;i++) {
					...
					div=="Depature"?$("#flightDepature .pagination").append(li):$("#flightRending .pagination").append(li);
				}
				if(res.pageMaker.next) div=="Depature"?$("#flightDepature .pagination").append(nextli):$("#flightRending .pagination").append(nextli);
		    },
		    error : function(request, status, error) { // 결과 에러 콜백함수
		        console.log(error)
		    }
	...
	$(function(){
		//항공편 출발 다이얼로그
		$(document).on("click","#flightDepature .index",function(){
			div = "Depature";
			page = $(this).attr("data-index");
			searchType = $("#flightDepature #searchType option:selected").val();
			keyword = $("#flightDepature #keywordInput").val();
			airAjax(div,page,searchType,keyword);
		})
		$(document).on("click","#flightDepature #prev",function(){
			div = "Depature";
			page = Number($("#flightDepature .index").eq(0).attr("data-index"))-1;
			searchType = $("#flightDepature #searchType option:selected").val();
			keyword = $("#flightDepature #keywordInput").val();
			airAjax(div,page,searchType,keyword);
		})
		$(document).on("click","#flightDepature #next",function(){
			div = "Depature";
			page = Number($("#flightDepature .index").eq(9).attr("data-index"))+1;
			searchType = $("#flightDepature #searchType option:selected").val();
			keyword = $("#flightDepature #keywordInput").val();
			airAjax(div,page,searchType,keyword);
		})
		...
		$("#flightDepature #btnSearch").click(function(){
			div = "Depature";
			page = $("#flightDepature .pagination").find(".active a").attr("data-index");
			searchType = $("#flightDepature #searchType option:selected").val();
			keyword = $("#flightDepature #keywordInput").val();
			airAjax(div,page,searchType,keyword);
		})
		...
		$(document).on("click","#flightRending #prev",function(){
			div = "Rending";
			page = Number($("#flightRending .index").eq(0).attr("data-index"))-1;
			searchType = $("#flightRending #searchType option:selected").val();
			keyword = $("#flightRending #keywordInput").val();
			airAjax(div,page,searchType,keyword);
		})
		$(document).on("click","#flightRending #next",function(){
			div = "Rending";
			page = Number($("#flightRending .index").eq(9).attr("data-index"))+1;
			searchType = $("#flightRending #searchType option:selected").val();
			keyword = $("#flightRending #keywordInput").val();
			airAjax(div,page,searchType,keyword);
		})
		...
		$("#flightRending #btnSearch").click(function(){
			div = "Depature";
			page = $("#flightRending .pagination").find(".active a").attr("data-index");
			searchType = $("#flightRending #searchType option:selected").val();
			keyword = $("#flightRending #keywordInput").val();
			airAjax(div,page,searchType,keyword);
		})
...
<body>
					...
					<form class="form-inline" action="addProductForm" method="post" enctype="multipart/form-data">
						<div class="group">
							<div class="form-group">
								...
								<div class="container">
									<button type="button" class="btn btn-primary" data-toggle="modal" id="showFlight">항공편 추가</button>
	...
	<div id="flightDepature" class="modal fade" role="dialog" data-backdrop="static" data-keyboard="false">
					...
					<div class="box-footer">
						<div class="text-center">
							<ul class="pagination">
								...
								<c:forEach begin="${pageMakerByFlightDepature.startPage }" end="${pageMakerByFlightDepature.endPage }" var="idx">
									<li class="${pageMakerByFlightDepature.cri.page == idx ?'active':''}"><a class="index" data-index="${idx }">${idx }</a></li>
								</c:forEach>
	...
	<div id="flightRending" class="modal fade" role="dialog" data-backdrop="static" data-keyboard="false">
	  	<div class="modal-dialog modal-lg">
		    <!-- Modal content-->
		    <div class="modal-content">
		      <div class="modal-header">
					...
					<div class="box-footer">
						<div class="text-center">
							<ul class="pagination">
								...
								<c:forEach begin="${pageMakerByFlightRending.startPage }" end="${pageMakerByFlightRending.endPage }" var="idx">
									<li class="${pageMakerByFlightRending.cri.page == idx ?'active':''}"><a class="index" data-index="${idx }">${idx }</a></li>
								</c:forEach>
