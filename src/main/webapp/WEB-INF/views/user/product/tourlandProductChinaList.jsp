	...
	$.ajax({
		...
		success : function(rs){
			 ...
			 if(rs.pageMaker.prev==true){
					var $li1 = $("<li>");
					var $a1 = $("<a>").attr("href", "${pageContext.request.contextPath}/tourlandProductChinaList?page=${pageMaker.startPage-1}" ).html("&laquo");
					$li1.append($a1);
				}
				...
				if(rs.pageMaker.next==true){
					var $li3 = $("<li>");
					var $a3 = $("<a>").attr("href", "${pageContext.request.contextPath}/tourlandProductChinaList?page=${pageMaker.startPage+1}" ).html("&laquo");
				...
				for(var j = rs.pageMaker.startPage; j<= rs.pageMaker.endPage; j++){
					...
					$a2 = $("<a>").attr("href", "${pageContext.request.contextPath}/tourlandProductChinaList?page="+j).html(j);
...
function getLowPriceList(page){
	$.ajax({
		...
		success : function(rs){
			 ...
			 if(rs.pageMaker.prev==true){
					var $li1 = $("<li>");
					var $a1 = $("<a>").attr("href", "${pageContext.request.contextPath}/tourlandProductChinaSearchLowPriceList?page=${pageMaker.startPage-1}" ).html("&laquo");
					$li1.append($a1);
				}
			...
			<div id="pkgListWrap">
				...
				<ul class="pagination">
					<c:if test="${pageMaker.prev == true}">
						<li><a href="${pageContext.request.contextPath}/tourlandProductChinaList?page=${pageMaker.startPage-1}">&laquo;</a></li>
					</c:if>
					<c:forEach begin="${pageMaker.startPage}" end="${pageMaker.endPage}" var="idx">
						<li class="${cri.page==idx?'active':''}"><a href="${pageContext.request.contextPath}/tourlandProductChinaList?page=${idx}">${idx}</a></li>
					</c:forEach>
					<c:if test="${pageMaker.next == true}">
						<li><a href="${pageContext.request.contextPath}/tourlandProductChinaList?page=${pageMaker.endPage+1}">&raquo;</a></li>
					</c:if>
				</ul>
			</div>
		</div>
